...
public class Townt_Projectile : Projectile
{
    ...
    void Update()
    {
        ...
        // transform.position = this.targetPos2;
        Vector3 nextPos = Vector3.MoveTowards(transform.position, targetPos, speed * Time.deltaTime);
        float baseY = Mathf.Lerp(transform.position.y, targetPos.y, 0.1f); //7*0.1 -> 0.7
        nextPos = new Vecotr3(nextPos.position.x, nextPos.position.y - baseY, transform.position.z);
        transform.position = nextPos;

        // 3) AddForce
        myRigidbody.AddForce(new Vector3(0,-1,0), ForceMode.Force);

        // 4) velocity?
        myRigidbody.velocity = new Vector(0,-1,0) * Time.deltaTime;
    }
